...
class BinarySearchTree:
    ...
    def __sorted_list_to_bst(self, nums, left, right):
        ...
        if currentValue is not None:
            currentNode = Node(currentValue)
            currentNode.left = self.__sorted_list_to_bst(nums,left,currentIndex)
            currentNode.right = self.__sorted_list_to_bst(nums,currentIndex,right)
        else:
            return None
        return currentNode
